...
1. **ทำงานในเบื้องหลัง (Background & System Tray):**
   ...
   - 💾 **[✨ ใหม่] จำการตั้งค่าอัตโนมัติ (Persistent Settings):** จำค่าปุ่มลัด, ฟอร์แมตไฟล์ และภาษาที่เลือกไว้ลงใน `%LocalAppData%\1ThaiAiCapture\settings.json` อัตโนมัติ เปิดโปรแกรมใหม่ไม่ต้องตั้งค่าซ้ำ
...
4. **แถบเครื่องมือลอยคู่บน-ล่าง (Top & Bottom Floating Toolbars):**
   ...
   - **แถบด้านล่าง (Bottom Bar - คำสั่งบันทึก/ใช้งาน):**
     - 📋 **คัดลอก (Copy)** : คัดลอกภาพไปยัง Clipboard (หรือกด `Enter`) พร้อมรองรับการวางรูปบน **Google Docs**, Web Browsers (Chrome, Edge) และแอป Chat (Discord, Slack) ด้วยสตรีม PNG แท้ ไม่หลุดไม่พัง
